  ...
  componentDidMount() {
    ...
    this.interval = setInterval(function() {
      fetch(
        ...
        .then(csvReceive => {
          console.log("code", csvReceive[0].code);
          ...
          const AreaChart = [];
          for (let i = 99; i >= 0; i--) {
            AreaChart.push([csvReceive[i].timestamp, csvReceive[i].tradePrice]);
          }
          console.log(AreaChart);
          console.log(csvReceive[99].timestamp);
          console.log(csvReceive[99].tradePrice);
          ...
          component.setState({
            options: CopyOptions,
            name: csvReceive[0].code.slice(15, 18),
            closeprice: numberWithCommas(csvReceive[0].tradePrice),
            chgrate: (csvReceive[0].signedChangeRate * 100).toFixed(2) + "%",
            acctradevolume: numberWithCommas(
              Math.floor(csvReceive[0].candleAccTradeVolume) + "개"
            ),
            acctradeprice: numberWithCommas(
              Math.floor(csvReceive[0].candleAccTradePrice / 1000000) + "백만"
            ),
            signedchangeprice:
              "(" + numberWithCommas(csvReceive[0].signedChangePrice) + ")"
          }); // state update
        });
    }, 1000);
  }
